...
pub fn compress_bytes(bytes: &[u8], field_type: ValueType) -> Result<Vec<u8>> {
    match field_type.get_optimal_compression() {
...
pub fn decompress_bytes(compressed_bytes: &[u8], field_type: ValueType) -> Result<Vec<u8>> {
    match field_type.get_optimal_compression() {
